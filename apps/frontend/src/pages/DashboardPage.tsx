            ...
            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              {hybridRecommendations.length === 0 ? (
                ...
                hybridRecommendations.map((movie) => (
                  <RecommendationCard key={`hybrid-${movie.movieId}`} movie={movie} />
                ))
              )}
            </div>
          </section>
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
              {preferenceRecommendations.length === 0 ? (
                ...
                preferenceRecommendations.map((movie) => (
                  <RecommendationCard key={`pref-${movie.movieId}`} movie={movie} />
                ))
              )}
            </div>
          </section>
...
function RecommendationCard({ movie }: { movie: RecommendationItem }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
      ...
      <div className="p-4">
        ...
        {movie.director && (
          <p className="mt-2 text-sm text-slate-500">Director: {movie.director}</p>
        )}

        {movie.overview && (
          <p className="mt-3 line-clamp-4 text-sm text-slate-700">{movie.overview}</p>
        )}
        ...
        <div className="mt-4 space-y-1 text-xs text-slate-500">
          ...
          <div className="flex items-center justify-between">
            ...
            <span>Support: {movie.support_count ?? 0}</span>
          </div>

          {"preference_score" in movie && (
            <div className="flex items-center justify-between">
              <span>Preference: {movie.preference_score ?? 0}</span>
              <span>Reranked: {movie.reranked_score ?? movie.final_score ?? 0}</span>
            </div>
          )}
        </div>
